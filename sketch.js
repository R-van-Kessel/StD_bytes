...
function generateQuestions() {
    ...
    // Eenheden in volgorde (factor 1000 tussen elke stap)
    ...
    // Converteer van bytes naar eenheid
    function fromBytes(bytes, unit) {
        const index = units.indexOf(unit);
        return bytes / Math.pow(1000, index);
    }

    for (let i = 0; i < 10; i++) {
        // Kies random operatie: optellen of aftrekken
        const operation = random() > 0.5 ? '+' : '-';
        
        // Kies random eenheden: maximaal 2 stappen verschil (factor 1.000.000)
        // Mogelijke combinaties: B-KB, KB-MB, MB-GB, GB-TB (1 stap)
        //                        B-MB, KB-GB, MB-TB (2 stappen)
        ...
        const combo = possibleCombos[floor(random(possibleCombos.length))];
        const indices = shuffle(combo); // Shuffle om random volgorde te krijgen
        let unit1 = units[indices[0]];
        let unit2 = units[indices[1]];
        
        // Kies antwoord eenheid: één van de twee eenheden uit de som
        const answerUnit = random() > 0.5 ? unit1 : unit2;
        
        let value1, value2, resultBytes;
        
        if (operation === '+') {
            // Optelling
            // Genereer waarden die mooie uitkomsten geven
            value1 = (floor(random(1, 20)) * 100) / 100; // 0.01 tot 19.99
            value2 = (floor(random(1, 20)) * 100) / 100;
            
            // Bereken in bytes
            const bytes1 = toBytes(value1, unit1);
            const bytes2 = toBytes(value2, unit2);
            resultBytes = bytes1 + bytes2;
            
        } else {
            // Aftrekking - zorg dat resultaat positief is
            value1 = (floor(random(10, 30)) * 100) / 100; // Grotere waarde
            value2 = (floor(random(1, 10)) * 100) / 100;  // Kleinere waarde
            
            // Bereken in bytes
            const bytes1 = toBytes(value1, unit1);
            const bytes2 = toBytes(value2, unit2);
            
            // Zorg dat resultaat positief is
            if (bytes1 > bytes2) {
                resultBytes = bytes1 - bytes2;
            } else {
                // Wissel om
                ...
                unit1 = unit2;
                unit2 = tempUnit;
                resultBytes = toBytes(value1, unit1) - toBytes(value2, unit2);
            }
        }
        
        // Converteer resultaat naar gewenste eenheid
        let result = fromBytes(resultBytes, answerUnit);
        
        // Rond af op maximaal 2 decimalen
        result = Math.round(result * 100) / 100;
        
        // Format met komma (voor decimalen, NIET voor duizendtallen)
        let value1Str = value1.toString().replace('.', ',');
        let value2Str = value2.toString().replace('.', ',');
        let resultStr = result.toString().replace('.', ',');
        
        let text = `${value1Str} ${unit1} ${operation} ${value2Str} ${unit2}\n= ? ${answerUnit}`;
        let answer = resultStr;
...
function drawBlock(block) {
    ...
    if (block.text.includes('/')) {
        // Splits breuk in teller en noemer
        ...
        // Kleinere tekst voor byte sommen
        textSize(12);
        let lineHeight = 16;
